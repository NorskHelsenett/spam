	...
	<aside class="relative hidden h-screen min-h-screen max-h-screen w-64 flex-shrink-0 flex-col overflow-y-auto bg-[var(--main-content-bg)] px-6 py-10 md:flex">
		...
		<nav class="mt-8 flex-1 space-y-2" aria-label="Primary">
			{#each navLinks as link}
				...
				>
					<span class="flex h-8 w-8 items-center justify-center rounded-full text-[var(--accent)]" aria-hidden="true">{@html navIcons[link.icon]}</span>
		...
		<div class="mt-auto flex flex-col gap-2 pt-6">
			...
			<button type="button" class="group flex items-center gap-2 rounded-full border border-transparent px-4 py-2 text-[0.9rem] text-[var(--text-secondary)] transition hover:bg-[var(--hover-bg-subtle)] hover:text-[var(--text-bright)]">
				<span class="flex h-8 w-8 items-center justify-center rounded-full text-[var(--accent)]" aria-hidden="true">{@html utilityIcons.account}</span>
