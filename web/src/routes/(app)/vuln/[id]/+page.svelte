<script lang="ts">
	...
	const repoPageLink = (repo: AffectedRepo): string | null => {
		...
		return `/providers/repo?provider=${encodeURIComponent(repo.provider)}&path=${encodeURIComponent(path)}&provider_id=${encodeURIComponent(repo.provider_instance_id)}`;
	};
								...
								<tr class={auth.is_primary ? 'bg-[var(--accent)]/5' : ''}>
									<td class="px-4 py-2 align-top">
										<div class="flex items-center gap-2">
											<a href="/vuln/{auth.vuln_id}" class="font-mono text-xs text-[var(--accent)] hover:underline">{auth.vuln_id}</a>
											{#if auth.is_primary}
							...
							<div class="rounded-2xl border border-[var(--border-color)]/60 bg-[var(--card-bg)]/40 p-4">
								...
								<div class="mt-2 flex flex-wrap gap-2">
									{#each aliases as alias}
										<a href="/vuln/{alias}" class="inline-flex items-center rounded-full border border-[var(--border-color)] bg-[var(--hover-bg)] px-2 py-0.5 font-mono text-xs text-[var(--text-secondary)] transition hover:text-[var(--accent)]">{alias}</a>
									{/each}
								</div>
							</div>
						{/if}
