					...
					<table class="min-w-full table-fixed divide-y divide-[var(--border-color)]/30 text-sm">
							...
							<tbody class="text-[var(--text-secondary)]">
								...
								{#each sortedImages.slice(imageVirt.start, imageVirt.end) as img}
									<tr
										class="border-b border-[var(--border-color)]/15 transition hover:bg-[var(--hover-bg-subtle)] hover:text-[var(--text-bright)] {img.digest_id ? 'cursor-pointer' : ''} {imageDrawerOpen && imageDrawerId === img.digest ? 'bg-[var(--hover-bg-subtle)]' : ''}"
										style="height:{ROW_HEIGHT}px;max-height:{ROW_HEIGHT}px"
										onclick={() => { if (img.digest_id) openImageDrawer(img.digest); }}
									>
										...
										<td class="truncate px-0 py-3 font-semibold text-[var(--text-bright)]" title={img.image}>
											{#if img.digest_id}
												<a class="hover:text-[var(--accent)] hover:underline" href={`/images/${img.digest}`} onclick={(e) => e.stopPropagation()}>{img.image}</a>
											{:else}
												{img.image}
											{/if}
										</td>
