-- Make the asset_risk MV honour the new VEX fields (revoked_at,
-- snooze_until, asset_scope) so per-CVE acknowledgments flow back into
-- the bucket math.
--
-- v1 scope:
--   * revoked_at IS NULL — append-only history; tombstoned rows stop
--     suppressing.
--   * snooze_until > NOW() — snoozes auto-expire at refresh time; the
--     materialised view is the staleness boundary, which is fine
--     because snoozes are days/weeks, not minutes.
--   * asset_scope — NULL / '' means global (legacy); 'image:<digest>'
--     suppresses one image only. Repo and cluster scopes are reserved
--     for follow-ups; non-empty unmatched scopes do not suppress at
--     all (fail-closed).
--
-- The asset-level ack (triage_acknowledgments) is NOT applied here.
-- It runs as a runtime filter in LoadTriage so snooze expiry takes
-- effect on the next API call, not the next refresh.
--
-- Body is otherwise identical to 20260510b. When you edit the upstream
-- pre-aggregation, mirror the change here too. (Long-term we should
-- split the body into a template and inject only the WHERE clauses.)
...
repo_vuln_canonical AS (
    ...
    WHERE NOT EXISTS (
        ...
        WHERE vex.status IN ('not_affected', 'fixed')
          AND vex.revoked_at IS NULL
          AND (vex.snooze_until IS NULL OR vex.snooze_until > NOW())
          AND (vex.asset_scope IS NULL OR vex.asset_scope = '')
...
image_vuln_canonical AS (
    ...
    WHERE NOT EXISTS (
        ...
        LEFT JOIN image_digests dx  ON dx.id::text    = v.image_id
        WHERE vex.status IN ('not_affected', 'fixed')
          AND vex.revoked_at IS NULL
          AND (vex.snooze_until IS NULL OR vex.snooze_until > NOW())
          AND (
              vex.asset_scope IS NULL
              OR vex.asset_scope = ''
              OR vex.asset_scope = ('image:' || dx.digest)
          )
          AND COALESCE(vmx.canonical_id, vex.vuln_id)
              = COALESCE(vm.canonical_id, v.vuln_id)
