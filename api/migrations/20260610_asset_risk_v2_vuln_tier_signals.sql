-- asset_risk v2: vuln-only tier signals.
--
-- The triage tiers move from composite Threat × Trust rules to a
-- vuln-driven decision matrix (KEV, EPSS, exposure, severity,
-- fixability). The Go side needs signals the v1 body didn't carry:
--
--   medium_count / low_count      full severity spectrum so the
--                                 "low_severity_only" deprioritized
--                                 reason can fire.
--   has_fix_for_high              fixability beyond criticals.
--   kev_fixable_count             KEV vulns with a fixed_version —
--                                 "exploited in the wild, patch
--                                 exists" is its own tier rule.
--   kev_ransomware_count          CISA known_ransomware KEVs.
--   kev_due_passed                any KEV past its BOD 22-01 due date.
--   kev_epss_max                  worst EPSS among KEV vulns.
--   exposed_kev_count             KEV vulns on an internet-exposed
--   exposed_critical_count        digest. For images these are exact
--   exposed_epss_max              (digest = exposure unit); for
--                                 clusters they come from a dedicated
--                                 pre-aggregation over exposed_digests
--                                 instead of the v1 approximation
--                                 (kev_count > 0 AND internet_exposed
--                                 could conflate a KEV on a
--                                 non-exposed image).
--   cluster_count /               image-only deployment spread: how
--   exposed_cluster_count         many clusters run the digest, and in
--                                 how many of them it is reachable
--                                 from the internet. One rebuild, N
--                                 redeploys.
--
-- Everything else is the 20260510b pre-aggregated body unchanged.
--
...
-- reapplies an earlier asset_risk creator and stops there — this file
-- is skipped on its hash match and the v2 body never replaces the old
-- one. This file supersedes 20260510b as the last asset_risk creator.
...
-- anyway. Then bound the wait with a lock_timeout so even if the
-- cancel signal doesn't propagate fast enough, bootstrap fails after
-- 60s instead of looping; the pod restarts and the next attempt wins.
DO $$
BEGIN
