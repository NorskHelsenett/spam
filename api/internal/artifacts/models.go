...
// CommitSHA is denormalized from RepoCommit for direct hash-based lookup — it survives
// repo deletions and does not depend on the RepoCommit UUID chain.
type SBOMBinding struct {
	...
	CommitSHA       string `gorm:"size:64;index:idx_sbom_binding_commit_sha"`
	CreatedAt       time.Time
